...
# Import Gradio conditionally to handle cases where it's not installed
try:
    import gradio as gr
...
class WebIde:
    ...
    def init_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler("web_ide.log"),
            ],
        )
        self.logger = logging.getLogger("web_ide")
        self.logger.info("Web IDE initialized")

    def list_projects(self) -> List[str]:
        """List all available projects."""
        return [dir.name for dir in self.projects_dir.iterdir() if dir.is_dir()]

    def init_ai(self, model_name: str, temperature: float, api_key: Optional[str] = None) -> str:
        ...
        try:
            ...
            return f"AI initialized with model {model_name}"
    ...
    def generate_code(self, prompt_text: str, status_callback=None) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            # Status update
            if status_callback:
                status_callback("Generating code...")
            ...
            # Status update
            if status_callback:
                status_callback("Code generation complete!")
    ...
    def improve_code(self, prompt_text: str, status_callback=None) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            # Status update
            if status_callback:
                status_callback("Improving code...")
            ...
            # Status update
            if status_callback:
                status_callback("Code improvement complete!")
    ...
    def run_code(self, status_callback=None) -> str:
        ...
        try:
            ...
            # Status update
            if status_callback:
                status_callback(f"Running {entrypoint}...")
            ...
            # Status update
            if status_callback:
                status_callback("Execution completed!")
    ...
    try:
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("AI Setup"):
                ...
                setup_button = gr.Button("Set Up AI", variant="primary")
                setup_output = gr.Markdown()

                setup_button.click(
                    fn=ide.init_ai,
                    inputs=[model_name, temperature, api_key],
        ...
        print(f"Error creating web UI: {str(e)}")
        ...
        with gr.Blocks(title="GPT Engineer - AI IDE (Fallback Mode)") as fallback_app:
            ...
            gr.Markdown("There was an error creating the full UI. This is a minimal fallback interface.")
            ...
            generate_btn.click(
                fn=lambda api, model, prompt: f"Would generate code with {model} based on: {prompt}",
...
def main():
    ...
    print("Starting GPT Engineer Web IDE...")
    ...
    try:
        ...
        app.launch(share=False)  # Changed to share=False for security by default
